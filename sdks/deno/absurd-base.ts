...
export class TaskContext {
  ...
  static async create(args: {
    ...
    for (const row of result.rows) {
      cache.set(row.checkpoint_name as string, row.state as JsonValue);
    }
...
export class AbsurdBase {
  ...
  registerTask<P = unknown, R = unknown>(
    ...
    this.registry.set(options.name, {
      ...
      handler: handler as TaskHandler<unknown, unknown>,
    });
  }
  ...
  async claimTasks(options?: {
    ...
    return result.rows as unknown as ClaimedTask[];
  }
